...
/** Optional bridge to Oculus' bundled Iris API. Unknown or failed APIs choose visual correctness over culling. */
public final class IrisCompatibility {
    ...
    public static void initialize(boolean irisLoaded) {
        ...
        try {
            ...
            available = true;
            Liubai.LOGGER.info("Oculus render-pass compatibility bridge initialized.");
        } catch (ReflectiveOperationException | RuntimeException error) {
            Liubai.LOGGER.warn("Oculus was detected but its Iris render-pass API was unavailable; Liubai will conservatively bypass per-object policies while Oculus is present.", error);
        }
    }
    ...
    private static synchronized void disableAfterFailure(Throwable error) {
        if (!available) return;
        available = false;
        Liubai.LOGGER.warn("Oculus render-pass lookup failed at runtime; Liubai will conservatively bypass per-object policies for this session.", error);
    }
}
