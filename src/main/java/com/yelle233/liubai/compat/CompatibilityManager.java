...
public final class CompatibilityManager {
    ...
    public void detect() {
        ...
        valkyrienskiesLoaded = mods.isLoaded("valkyrienskies");
        irisLoaded = mods.isLoaded("oculus");
        ...
        if (flywheelLoaded && !supportsAdaptiveFlywheelLimiter()) {
            ...
            Liubai.LOGGER.info("Flywheel adaptive limiter enabled for the supported Create 6.0.8 / Flywheel 1.0.5 line.");
        }
        if (valkyrienskiesLoaded) {
            Liubai.LOGGER.info("Valkyrien Skies {} detected; ship-managed objects will bypass Liubai per-object render policies, and ownerless hooks use a conservative safe mode.", valkyrienskiesVersion);
        }
    }
