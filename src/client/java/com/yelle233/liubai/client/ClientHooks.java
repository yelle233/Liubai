...
public final class ClientHooks {
    ...
    public static boolean shouldSkipEntity(Entity entity, AABB rendererBounds,
        ...
        if (!camera.isInitialized() || camera.position().distanceToSqr(cameraX, cameraY, cameraZ) > 0.01) return false;
        LiubaiClientSystem.INSTANCE.prepareMainRender(camera);
        return LiubaiClientSystem.INSTANCE.policies().decide(entity, rendererBounds).skip();
    }
    ...
    public static boolean shouldSkipNameTag(Entity entity) {
        if (IrisCompatibility.shouldBypassPerObjectPolicies()) return false;
        prepareMainRender();
        return LiubaiClientSystem.INSTANCE.policies().skipNameTag(entity);
    }

    public static boolean shouldSkipParticle(Particle particle) {
        return LiubaiClientSystem.INSTANCE.policies().skipParticle(particle.getBoundingBox().getCenter());
    }
