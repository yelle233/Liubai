...
public final class RenderStatistics {
    ...
    private long[] window = new long[16];
    ...
    public void sableEntityBypassed(){sableEntitiesBypassed++;} public void sableBlockEntityBypassed(){sableBlockEntitiesBypassed++;} public void sableParticleBypassed(){sableParticlesBypassed++;}
    ...
    public void finishFrame(VisibilityService.Counts visibility, int liveParticles) {
        ...
        int[] values={entitiesTested,entitiesSkipped,entitiesOccluded,entitiesTooSmall,entitiesDensityLimited,blockEntitiesTested,blockEntitiesSkipped,blockEntitiesOccluded,shadowsSkipped,nameTagsSkipped,particlesAccepted,particlesSkipped,temporalUpdatesDeferred,sableEntitiesBypassed,sableBlockEntitiesBypassed,sableParticlesBypassed};
        for(int i=0;i<values.length;i++)window[i]+=values[i]; long now=System.nanoTime(),elapsed=now-windowStartNanos;
        if(elapsed>=1_000_000_000L){double seconds=elapsed/1_000_000_000.0;int[] perSecond=new int[window.length];for(int i=0;i<window.length;i++)perSecond[i]=(int)Math.round(window[i]/seconds);rate=new Rate(perSecond);window=new long[16];windowStartNanos=now;}
        entitiesTested=entitiesSkipped=entitiesOccluded=entitiesTooSmall=entitiesDensityLimited=0;blockEntitiesTested=blockEntitiesSkipped=blockEntitiesOccluded=0;shadowsSkipped=nameTagsSkipped=particlesAccepted=particlesSkipped=temporalUpdatesDeferred=0;sableEntitiesBypassed=sableBlockEntitiesBypassed=sableParticlesBypassed=0;
    }
    public Snapshot snapshot(){return new Snapshot(rate.entitiesTested,rate.entitiesSkipped,rate.entitiesOccluded,rate.entitiesTooSmall,rate.entitiesDensityLimited,rate.blockEntitiesTested,rate.blockEntitiesSkipped,rate.blockEntitiesOccluded,rate.shadowsSkipped,rate.nameTagsSkipped,rate.particlesAccepted,rate.particlesSkipped,rate.temporalUpdatesDeferred,rate.sableEntitiesBypassed,rate.sableBlockEntitiesBypassed,rate.sableParticlesBypassed,visibility.visible(),visibility.occluded(),visibility.unknown(),visibility.queued(),visibility.checked(),visibility.timedOut(),visibility.micros(),liveParticles);}
    private record Rate(int entitiesTested,int entitiesSkipped,int entitiesOccluded,int entitiesTooSmall,int entitiesDensityLimited,int blockEntitiesTested,int blockEntitiesSkipped,int blockEntitiesOccluded,int shadowsSkipped,int nameTagsSkipped,int particlesAccepted,int particlesSkipped,int temporalUpdatesDeferred,int sableEntitiesBypassed,int sableBlockEntitiesBypassed,int sableParticlesBypassed){private static final Rate EMPTY=new Rate(new int[16]);private Rate(int[]v){this(v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8],v[9],v[10],v[11],v[12],v[13],v[14],v[15]);}}
    public record Snapshot(int entitiesTested,int entitiesSkipped,int entitiesOccluded,int entitiesTooSmall,int entitiesDensityLimited,int blockEntitiesTested,int blockEntitiesSkipped,int blockEntitiesOccluded,int shadowsSkipped,int nameTagsSkipped,int particlesAccepted,int particlesSkipped,int temporalUpdatesDeferred,int sableEntitiesBypassed,int sableBlockEntitiesBypassed,int sableParticlesBypassed,int visible,int occluded,int unknown,int queued,int visibilityChecks,int visibilityTimeouts,long visibilityMicros,int liveParticles){}
}
